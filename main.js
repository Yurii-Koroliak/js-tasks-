...
// Рядом с дорогой стоят столбы. Расстояние между столбами одинаковое, ширина столбов одинаковая. Ваша функция принимает три аргумента:
// количество столбов (≥ 1);
// расстояние между столбами (10 - 30 метров);
// ширина столба (10 - 50 сантиметров).
// Вычислите расстояние между первым и последним столбом в сантиметрах (без учета ширины первого и последнего столба).


function pillars(numPill, dist, width) {
  // your code here
  let betweenDist = dist*100;
  let distance;
  if (numPill <= 1) {
    return 0;
  } else {  
      distance = (betweenDist*(numPill-1))+(width*(numPill-2));
  }
  return distance;
}